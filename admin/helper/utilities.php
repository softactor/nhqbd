...
function image_upload_process($param) {
...
// Check if image file is a actual image or fake image
    ...
    if ($check !== false) {
        ...
        array_push($errorMessageContainer, "File is not an image.");
        $uploadOk = 0;
    }

// Check if file already exists
    if (file_exists($target_file)) {
        array_push($errorMessageContainer, "File already exists.");
        $uploadOk = 0;
    }

// Check file size
    if ($_FILES[$fieldName]["size"] > 500000) {
        array_push($errorMessageContainer, "File is too large.");
        $uploadOk = 0;
    }

// Allow certain file formats
    if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
        array_push($errorMessageContainer, "only JPG, JPEG, PNG & GIF files are allowed.");
        $uploadOk = 0;
    }
...
// Check if $uploadOk is set to 0 by an error
    if ($uploadOk == 0) {
        ...
        foreach($errorMessageContainer as $msg){
            $message.=$sl++.'. '.$msg;
        }
// if everything is ok, try to upload file
        ...
        if (move_uploaded_file($_FILES[$fieldName]["tmp_name"], $target_file)) {
            $status     =   'success';
            $message.="The file " . htmlspecialchars(basename($_FILES[$fieldName]["name"])) . " has been uploaded.";
        } else {
            $status     =   'error';
